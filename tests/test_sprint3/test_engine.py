...
class TestCognitiveEngine:
    ...
    def test_observation_logging_disabled(self):
        engine = CognitiveEngine(observation_logging=False, use_real_usd=False, in_memory=True)
        engine.process_exchange("twin_coach", {})
        stats = engine.get_buffer_stats()
    ...
    def test_prediction_disabled(self):
        engine = CognitiveEngine(prediction_enabled=False, use_real_usd=False, in_memory=True)
        for i in range(5):
    ...
    def test_graceful_without_model(self):
        engine = CognitiveEngine(model_path="nonexistent.joblib", use_real_usd=False, in_memory=True)
        result = engine.process_exchange("twin_coach", {})
